...
ordinario_tot_C <- SDO_ordinario %>% 
  dplyr::filter(TIPO == "C",
                ANNO == 2019) %>% 
  dplyr::pull(DIMISSIONI) %>% 
  sum()

diurno_tot_C <- SDO_diurno %>% 
  dplyr::filter(TIPO == "C",
                ANNO == 2019) %>% 
  dplyr::pull(DIMISSIONI) %>% 
  sum()
...
surgical_ita <- (diurno_tot_C + ordinario_tot_C) / (ita2019_M + ita2019_F) * 1000
surgical_eng <- (incidence_England_tot$MALE + incidence_England_tot$FEMALE) *  1000
incidence_it_en <- tibble(Incidence = c(surgical_ita, surgical_eng),
                          Country = c("Italy", "England"))
